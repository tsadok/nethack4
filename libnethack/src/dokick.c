    ...
    *kickobj_p = kickobj;
    ...
    if (slide && !Blind)
        pline("Whee!  %s %s %s the %s.", Doname2(kickobj),
              ...
              Is_waterlevel(level) ? "bubble" : surface(x, y));
...
dokick(const struct nh_cmd_arg *arg)
{
    ...
    if (Levitation) {
        ...
        if (isok(xx, yy) && !IS_ROCK(level->locations[xx][yy].typ) &&
            ...
            (!Is_airlevel(level) || !OBJ_AT(xx, yy))) {
    ...
    if (OBJ_AT(x, y) &&
        (!Levitation || Is_airlevel(level) || Is_waterlevel(level)
    ...
    dumb:
        ...
        if ((Is_airlevel(level) || Levitation) && rn2(2))
            hurtle(-dx, -dy, 1, TRUE);
