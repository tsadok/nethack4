...
static struct level *drop_to(schar);
    ...
    *kickobj_p = kickobj;
    ...
    if (slide && !Blind)
        pline("Whee!  %s %s %s the %s.", Doname2(kickobj),
              ...
              level == sp_lev(sl_water) ? "bubble" : surface(x, y));
...
dokick(const struct nh_cmd_arg *arg)
{
    ...
    if (Levitation) {
        ...
        if (isok(xx, yy) && !IS_ROCK(level->locations[xx][yy].typ) &&
            ...
            (level != sp_lev(sl_air) || !OBJ_AT(xx, yy))) {
    ...
    if (OBJ_AT(x, y) &&
        (!Levitation || level == sp_lev(sl_air) || level == sp_lev(sl_water)
    ...
    if (!IS_DOOR(maploc->typ)) {
        if (maploc->typ == SDOOR) {
            if (!Levitation && rn2(30) < avrg_attrib) {
                cvt_sdoor_to_door(maploc, level);       /* ->typ = DOOR */
        ...
        if (IS_THRONE(maploc->typ)) {
            ...
            if ((Luck < 0 || maploc->doormask) && !rn2(3)) {
                ...
                if (level->dlevel < level->dgn->num_dunlevs) {
                    fall_through(FALSE);
                    return 1;
                } else
                    goto ouch;
    ...
    dumb:
        ...
        if ((level == sp_lev(sl_air) || Levitation) && rn2(2))
            hurtle(-dx, -dy, 1, TRUE);
...
drop_to(schar loc)
{
    ...
    case MIGR_RANDOM:  /* trap door or hole */
        if (level != sp_lev(sl_castle) &&
            (In_endgame(level) || Is_botlevel(level)))
            return NULL;
        /* else fall to the next cases */
    case MIGR_STAIRS_UP:
    case MIGR_LADDER_UP:
        return level_below(level);
    case MIGR_SSTAIRS:
        return level->sstairs.tolev;
    default:
    case MIGR_NOWHERE:
        return NULL;
    }
...
void
impact_drop(struct obj *missile, xchar x, xchar y, struct level *dest)
{
    schar toloc;
    struct obj *obj, *obj2;
    struct monst *shkp;
    ...
    for (obj = level->objects[x][y]; obj; obj = obj2) {
        ...
        deliver_object(obj, level, toloc);
    }
...
void
deliver_object(struct obj *obj, struct level *lev, int where)
{
    int nx, ny;
...
down_gate(xchar x, xchar y)
{
    ...
    if (level == sp_lev(sl_quest_start) && !ok_to_quest(FALSE))
        return MIGR_NOWHERE;
