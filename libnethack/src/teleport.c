...
tele_impl(boolean wizard_tele, boolean run_next_to_u)
{
    ...
    if ((Uhave_amulet || On_W_tower_level(level)) && !rn2(3)) {
...
level_tele_impl(boolean wizard_tele)
{
    ...
    if ((Uhave_amulet || In_endgame(level) || In_sokoban(level))
        && !wizard_tele) {
            ...
            struct monst *mtmp)
{
    ...
    if (!xx) {
        ...
        if (level->dndest.nlx && On_W_tower_level(level))
                 ...
                 int in_sight)
{
    ...
    if (teleport_pet(mtmp, force_it)) {
        ...
        if ((tt == HOLE || tt == TRAPDOOR)) {
            if (level != sp_lev(sl_castle) && Is_botlevel(level)) {
